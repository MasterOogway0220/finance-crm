          ...
          <tbody>
            {loading ? (
              Array.from({ length: 8 }).map((_, i) => (
                <tr key={i}><td colSpan={!readOnly ? 10 : 9} className="px-4 py-2"><Skeleton className="h-8 w-full" /></td></tr>
              ))
            ) : clients.length === 0 ? (
              <tr><td colSpan={!readOnly ? 10 : 9} className="px-4 py-12 text-center text-gray-400">
