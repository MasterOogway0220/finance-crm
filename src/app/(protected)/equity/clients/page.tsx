...
import { Search, Download, CalendarIcon, Users, AlertTriangle } from 'lucide-react'
    ...
    <div className="page-container space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-3">
        <div className="flex flex-wrap gap-2 items-center">
          <div className={cn('relative flex-1 min-w-48', inactive2m && 'opacity-40 pointer-events-none')}>
          ...
          <Button
            size="sm"
            variant={inactive2m ? 'default' : 'outline'}
            onClick={toggleInactive2m}
            className={cn('gap-1.5', inactive2m && 'bg-orange-500 hover:bg-orange-600 border-orange-500')}
          >
            <AlertTriangle className="h-3.5 w-3.5" />
            No Business 2+ Months
          </Button>
      ...
      {inactive2m && (
        <div className="flex items-center gap-2 px-3 py-2 bg-orange-50 border border-orange-200 rounded-lg text-sm text-orange-700">
          ...
          Showing equity clients with no business (Equity or MF) since{' '}
          <span className="font-semibold">{cutoffDate.toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
        </div>
      )}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              clients.map((client) => <InactiveClientRow key={client.id} client={client} />)
            ) : (
              clients.map((client) => (
                <ClientRow
                  key={client.id}
...
function InactiveClientRow({ client }: { client: ClientWithOperator }) {
  return (
